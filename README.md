...
### 管理端功能（`/admin`）
- 后台分为两个页签：**🔗 短链管理**（统计、趋势图、站点设置、所有短链列表）和 **👥 用户管理**（用户列表）。每个列表默认显示 30 条，可在右上角输入每页显示数量（10-500 条），避免页面过长。
...
| POST | `/api/admin/users/status` | 用户审核/状态管理（active=通过/解封，pending=待审核，blocked=封禁并注销会话） | 管理员 Token |
