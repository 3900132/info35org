...
- **登录 / 注册成功后自动跳转到用户中心**（`/user-center`）：分页展示当前账户全部短链（短地址、原始长链/文字内容、点击统计、创建时间），默认每页 30 条、可输入每页数量（1-500），支持翻页与刷新；首页的账户名与「我的短链」按钮也会链接到该页面。
- 未登录时行为与原版完全一致（在开关关闭的情况下）。

### 管理端功能（`/admin`）
- 后台仪表盘包含：统计卡片、点击趋势图、站点设置、存储与跳转设置、用户管理页签；「📋 所有短链」与「🚩 举报反馈」已拆分为独立页面，避免仪表盘过长。
- **`/admin-links`（所有短链）**：全部短链的分页列表（默认每页 30 条、可输入每页数量），支持搜索过滤、批量删除、单条删除、复制短链、生成/下载二维码、文字内容预览、CSV 导出。
- 「站点设置」提供两个开关：**仅注册用户可生成短链**、**新用户注册需管理员审核**（开启后新注册账户为"待审核"状态，管理员通过前无法生成短链）。
- 「存储与跳转设置」提供两项可调参数与一个手动清理按钮：
  - **非注册用户短链保留天数**（默认 7，0 = 永久保留）：访客（未注册用户）生成的短链超过该天数后自动从 KV 删除（即使创建时选了更长的有效期，也按此上限清理）；注册用户的短链不受影响。
  - **跳转中间页停留秒数**（默认 3，0 = 立即跳转）：打开短链后的中间页按该秒数倒计时跳转，并提供"立即跳转"按钮。
  - **🧹 立即清理 KV**：手动触发一次清理，清除过期短链、超保留期的访客短链、失效的限流记录与 30 天前的历史统计数据。
  - 此外系统会在用户访问/生成短链时**后台自动执行清理**（约每 6 小时一轮，无需人工干预）。
- **`/admin-reports`（举报反馈）**：展示用户通过跳转中间页"🚩 举报此链接"提交的举报（含短地址、目标链接/内容、举报原因、时间），分页展示（默认每页 30 条、可输入每页数量），处理完成后可删除记录。
- 「用户管理」列出所有注册用户：邮箱（账户）、状态（正常/待审核/已封禁）、注册时间、短链数量、累计点击，并支持**通过审核、封禁（立即注销其全部会话）、解封、取消资格、删除**（删除后其短链保留，仅解除归属并注销其全部会话）。
- **点击邮箱（账户）可跳转到独立页**（`/admin-user?username=xxx`），分页查看该用户生成的所有短链明细：短地址、类型、原始长链/内容、每条短链的点击统计与创建时间。

### 密码与会话安全
- **邮箱即账户**：系统不设独立用户名，注册与登录均使用邮箱；每个邮箱只能绑定一个账号。
- 注册强制 **邮箱验证码验证**：验证码 6 位数字、5 分钟有效、同邮箱 60 秒限发一封、每 IP 每小时限 10 封、单码最多 5 次尝试，验证通过后立即销毁。
...
| GET/POST | `/api/admin/settings` | 获取/设置站点设置（注册开关 + 非注册用户短链保留天数 + 跳转中间页停留秒数） | 管理员 Token |
| GET | `/api/admin/users` | 管理员获取用户列表及统计 | 管理员 Token |
| GET | `/api/admin/user-links?username=xxx` | 管理员下钻查看某用户全部短链 | 管理员 Token |
| POST | `/api/admin/user-status` | 用户审核/状态管理（active=通过/解封，pending=待审核，blocked=封禁并注销会话） | 管理员 Token |
| DELETE/POST | `/api/admin/user-delete` | 管理员删除用户（支持批量） | 管理员 Token |
| POST | `/api/report` | 用户举报短链（跳转中间页提交，含短链与目标内容） | 无 |
| GET | `/api/admin/reports` | 管理员获取全部举报反馈 | 管理员 Token |
| DELETE/POST | `/api/admin/report-delete` | 管理员删除举报记录 | 管理员 Token |
| POST | `/api/admin/cleanup` | 管理员手动清理 KV（过期短链/失效限流记录/过期统计） | 管理员 Token |
...
- **发送失败（502）**：查看 EdgeOne Pages 的函数日志，或看页面报错中的错误码：`InvalidAccessKeyId` = AccessKey ID 错误；`SignatureDoesNotMatch` = AccessKey Secret 错误；`InvalidMailAddress.SendObject` = 发信地址未验证；`InvalidSendMailDomain` / 域名相关 = 发信域名 DNS 未验证生效。
- **阿里云技术说明**：本项目通过 DirectMail 官方 RPC API（POST 表单编码 + HMAC-SHA1 签名）发送，与 SMTP 发信效果等同。
...
│   │   ├── register.js           # 用户注册 API (邮箱验证码校验 + PBKDF2 密码哈希)
    ...
    │       └── users/
    │           ├── status.js       # 管理员用户审核/封禁/解封 API (封禁同时注销全部会话)
