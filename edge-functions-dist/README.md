...
- **用户注册体系（可选开关）**：管理员可自由切换"仅注册用户可生成短链"或"所有访客均可生成"两种模式；开启后未登录访客将被拒绝生成并引导注册登录。注册必须通过**邮箱验证码验证**。注册用户生成短链后自动归属到其账户，可随时在"我的短链"面板查看长短链与实时点击统计。
- **用户管理后台**：管理员可查看所有注册用户列表（含每人短链数量与累计点击），**点击用户名即可下钻查看该用户生成的全部短链明细（短链/长链及每条点击统计）**，并支持删除用户（其短链保留）。
...
### 用户端功能
- 首页提供 **登录 / 注册** 面板，注册成功后自动登录（会话 Token 有效期 7 天）。
- 登录后生成的短链会自动归属到当前账户，首页「我的短链」面板实时列出该用户全部短链（短地址、原始长链/文字内容、点击统计、创建时间）。
- 未登录时行为与原版完全一致（在开关关闭的情况下）。

### 管理端功能（`/admin`）
- 「用户管理」卡片列出所有注册用户：用户名、注册时间、短链数量、累计点击，并支持删除用户（删除后其短链保留，仅解除归属并注销其全部会话）。
- **点击用户名即可下钻**，弹窗展示该用户生成的所有短链明细：短地址、类型、原始长链/内容、每条短链的点击统计与创建时间。

### 密码与会话安全
- 注册强制 **邮箱验证码验证**：验证码 6 位数字、10 分钟有效、同邮箱 60 秒限发一封、每 IP 每小时限 10 封、单码最多 5 次尝试，验证通过后立即销毁。
- 每个邮箱只能绑定一个账号，密码使用 **PBKDF2 (SHA-256，10 万次迭代 + 每用户随机盐)** 加密后存储，数据库泄露也不会暴露明文密码。
...
| GET | `/api/auth/me` | 获取当前登录用户名 | 用户 Token |
